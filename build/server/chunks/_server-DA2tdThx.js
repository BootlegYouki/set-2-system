  ...
  try {
    ...
    const activities = result.map((row) => {
      ...
      switch (activityType) {
        ...
        case "assignment":
          const assignedSectionNames = data.section_names ? Array.isArray(data.section_names) ? data.section_names.join(", ") : data.section_names : data.sections || "sections";
        ...
        case "assignment_return":
          const unassignedSectionNames = data.unassigned_sections ? data.unassigned_sections.map((s) => s.name).join(", ") : data.section_names || data.sections || "all sections";
...
export { GET, POST };
//# sourceMappingURL=_server-DA2tdThx.js.map
